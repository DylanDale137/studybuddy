...
class Subject4Component(Subject4ComponentTemplate):
  ...
  def file_loader_subject1_change(self, file, **event_args):
    ...
    if not user:
      ...
      return
    anvil.server.call("add_file", file, "subject4")
    self.load_files()
  ...
  def load_files(self):
    ...
    files = anvil.server.call("get_files", "subject4", group_name)
  ...
  def load_messages(self):
    ...
    if not user:
      return
    group_name = user["group_name"]
    messages = anvil.server.call_s("get_messages", "subject4", group_name)
  ...
  def button_send_click(self, **event_args):
    ...
    anvil.server.call_s(
      "add_message", user["first_name"], message, "subject4", local_time
    )
