from ._anvil_designer import Subject6ComponentTemplate
from anvil import *
import anvil.users
...
class Subject6Component(Subject6ComponentTemplate):
  ...
  def load_subject(self):
    subject = anvil.server.call("get_subject_by_order", 6)
    if subject:
  ...
  def file_loader_subject1_change(self, file, **event_args):
    ...
    if not user:
      ...
      return
    anvil.server.call("add_file", file, "subject6")
    self.load_files()
  ...
  def load_files(self):
    ...
    files = anvil.server.call("get_files", "subject6", group_name)
  ...
  def load_messages(self):
    ...
    if not user:
      return
    group_name = user["group_name"]
    messages = anvil.server.call_s("get_messages", "subject6", group_name)
  ...
  def button_send_click(self, **event_args):
    ...
    anvil.server.call_s(
      "add_message", user["first_name"], message, "subject6", local_time
    )
