from ._anvil_designer import Subject7ComponentTemplate
from anvil import *
import anvil.users
...
class Subject7Component(Subject7ComponentTemplate):
  ...
  def load_subject(self):
    subject = anvil.server.call("get_subject_by_order", 7)
    if subject:
  ...
  def file_loader_subject1_change(self, file, **event_args):
    ...
    if not user:
      ...
      return
    anvil.server.call("add_file", file, "subject7")
    self.load_files()
  ...
  def load_files(self):
    ...
    files = anvil.server.call("get_files", "subject7", group_name)
  ...
  def button_send_click(self, **event_args):
    ...
    anvil.server.call_s(
      "add_message", user["first_name"], message, "subject7", local_time
    )
